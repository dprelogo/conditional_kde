...
class TestIntegration:
    ...
    def bimodal_data(self):
        ...
        # Two clusters
        cluster1 = np.random.multivariate_normal([0, 0, 0], np.eye(3) * 0.5, n_samples//2)
        cluster2 = np.random.multivariate_normal([5, 5, 5], np.eye(3) * 0.5, n_samples//2)
    ...
    def test_gaussian_vs_kde_single_gaussian(self, correlated_data):
        ...
        log_probs_cg_cond = cg.score_samples(test_points, conditional_features=['a', 'b'])
        log_probs_kde_cond = kde.score_samples(test_points, conditional_features=['a', 'b'])
        
        correlation_cond = np.corrcoef(log_probs_cg_cond, log_probs_kde_cond)[0, 1]
        assert correlation_cond > 0.05  # Very relaxed threshold - methods are quite different
    ...
    def test_conditional_sampling_consistency(self, correlated_data):
        ...
        samples_cg = cg.sample(conditionals=conditionals, n_samples=1000, random_state=42)
        samples_kde = kde.sample(conditionals=conditionals, n_samples=1000, random_state=42)
    ...
    def test_edge_cases_and_errors(self):
        ...
        with pytest.raises(ValueError, match="conditional_features should be in features"):
