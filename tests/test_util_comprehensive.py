...
class TestDataWhitenerComprehensive:
    ...
    def test_init_invalid_algorithm(self):
        ...
        with pytest.raises(ValueError, match="algorithm should be None, center, rescale, PCA or ZCA"):
    ...
    def test_fit_weights_validation(self, simple_data):
        ...
        # Wrong weight length
        with pytest.raises(ValueError, match="Weights and X should be of the same length"):
...
class TestInterpolatorComprehensive:
    ...
    def test_shape_preservation(self, grid_2d):
        ...
        for shape in shapes:
            ...
            assert result.shape == expected_shape or (result.shape == () and expected_shape == (1,))
    ...
    def test_edge_case_single_point_grid(self):
        ...
        xi_out = np.array([1.0])
        result_out = interp_no_bounds(xi_out, return_aux=False)
