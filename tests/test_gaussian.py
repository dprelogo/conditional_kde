...
from conditional_kde.gaussian import ConditionalGaussian, ConditionalGaussianKernelDensity
...
class TestConditionalGaussian:
    ...
    def test_score_samples_conditional(self, simple_3d_data):
        ...
        log_probs = cg.score_samples(simple_3d_data[:10], conditional_features=['a', 'c'])
        assert log_probs.shape == (10,)
    ...
    def test_check_conditionals(self):
        ...
        with pytest.raises(ValueError):
            cg._check_conditionals({'a': np.array([1.0]), 'b': np.array([1.0, 2.0])}, 10)
...
class TestConditionalGaussianKernelDensity:
    ...
    def test_sample_conditional_rescale(self, simple_3d_data):
        ...
        assert np.all(np.abs(samples[:, 0] - 1.0) < 0.1)  # Should be close to conditional value
